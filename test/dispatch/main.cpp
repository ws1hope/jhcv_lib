...
int main(int argc, char* argv[])
{
    ...
    if (!test_mode) {
        std::cout << "=== Dispatch HTTP Service ===" << std::endl;
    }
    std::cout << "[INFO] Config: " << config_path << std::endl;

    auto start_time = std::chrono::high_resolution_clock::now();
    std::unique_ptr<JHDeepCore::DispatchService> service_ptr;
    try {
        service_ptr = std::make_unique<JHDeepCore::DispatchService>(config_path);
    } catch (const std::exception& e) {
        std::cerr << "[ERROR] Failed to initialize service: " << e.what() << std::endl;
        return 1;
    }
    JHDeepCore::DispatchService& service = *service_ptr;
    ...
    std::cout << "[INFO] Default branch: " << service.config().default_branch << std::endl;
    ...
    server.Get(prefix + "/hello", [](const httplib::Request&, httplib::Response& resp) {
        resp.set_content("Hello World! Dispatch service running.", "text/plain");
        resp.status = 200;
    });
