...
static void printHelp(const char* progName)
{
    ...
    std::cout << "  -H, --heat     Heat number for local test (10/11 digits or 6-digit A-F prefix, selects roi1-roiN)" << std::endl;
...
int main(int argc, char* argv[])
{
    ...
    if (!test_mode) {
        std::cout << "=== Fujian HTTP Service ===" << std::endl;
    }
    std::cout << "[INFO] Config: " << config_path << std::endl;

    auto start_time = std::chrono::high_resolution_clock::now();
    std::unique_ptr<JHDeepCore::FujianService> service_ptr;
    try {
        service_ptr = std::make_unique<JHDeepCore::FujianService>(config_path);
    } catch (const std::exception& e) {
        std::cerr << "[ERROR] Failed to initialize service: " << e.what() << std::endl;
        return 1;
    }
    JHDeepCore::FujianService& service = *service_ptr;
    auto end_time = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time);
    std::cout << "[INFO] Service init time: " << duration.count() << " ms" << std::endl;
    std::cout << "[INFO] Stations: " << service.config().stations.size()
    ...
    std::string prefix = "/" + service.config().service_name;

    server.Get(prefix + "/hello", [](const httplib::Request&, httplib::Response& resp) {
        resp.set_content("Hello World! Fujian service running.", "text/plain");
        resp.status = 200;
    });
    ...
    server.Get(prefix + "/stop", [&](const httplib::Request&, httplib::Response&) {
