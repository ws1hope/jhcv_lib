...
int main(int argc, char* argv[])
{
    ...
    if (!test_mode) {
        std::cout << "=== Huaxin HTTP Service ===" << std::endl;
    }
    std::cout << "[INFO] Config: " << config_path << std::endl;

    auto start_time = std::chrono::high_resolution_clock::now();
    std::unique_ptr<JHDeepCore::HuaxinService> service_ptr;
    try {
        service_ptr = std::make_unique<JHDeepCore::HuaxinService>(config_path);
    } catch (const std::exception& e) {
        std::cerr << "[ERROR] Failed to initialize models: " << e.what() << std::endl;
        return 1;
    }
    JHDeepCore::HuaxinService& service = *service_ptr;
    ...
    // 端点路径完全来自 config 的 url_path，无任何自带后缀
    std::string url = service.config().url_path;
    if (url.empty() || url[0] != '/') {
        url = "/" + url;
    }
    ...
    std::cout << "[INFO] POST " << url << std::endl;
