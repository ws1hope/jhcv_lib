...
// 特带卷曲多相机盘卷定位编排（旧 HTTP 服务算法核心的等价迁移）。
//
// 等价迁移约定（详见 docs/jhcv_lib集成方案.md §0 冻结决策）：
// - 阈值、处理顺序、坐标规则与旧服务一致，第一阶段不改算法行为。
...
class TedaiJuanquPipeline {
  ...
  private:
    LegacyReelDetector *detector(int camera_id);  // 按相机懒加载并缓存
    LegacyReelClassifier *classifier();           // 懒加载并缓存
    ...
    std::map<int, std::unique_ptr<LegacyReelDetector>> detectors_;
    std::unique_ptr<LegacyReelClassifier> classifier_;
