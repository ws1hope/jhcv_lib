...
**Windows**：第三方库统一放在 `G:/3thirdparty/`（opencv / onnxruntime-1.18.0-cuda12.2-win-x64 / tensorrt / paddle_inference），CUDA 在 `C:/Program Files/NVIDIA GPU Computing Toolkit/CUDA/v12.2`。运行时需把上述 DLL 加入 `PATH`（见 `.vscode/launch.json`）。
