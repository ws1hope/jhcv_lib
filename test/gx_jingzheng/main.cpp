...
int main(int argc, char* argv[])
{
    ...
    if (!test_mode) {
        std::cout << "=== GxJingzheng HTTP Service ===" << std::endl;
    }
    std::cout << "[INFO] Config: " << config_path << std::endl;

    auto start_time = std::chrono::high_resolution_clock::now();
    std::unique_ptr<JHDeepCore::GxJingzhengService> service_ptr;
    try {
        service_ptr = std::make_unique<JHDeepCore::GxJingzhengService>(config_path);
    } catch (const std::exception& e) {
        std::cerr << "[ERROR] Failed to initialize models: " << e.what() << std::endl;
        return 1;
    }
    JHDeepCore::GxJingzhengService& service = *service_ptr;
    ...
    server.Get(prefix + "/hello", [](const httplib::Request&, httplib::Response& resp) {
        resp.set_content("Hello World! gx_jingzheng service running.", "text/plain");
        resp.status = 200;
    });
